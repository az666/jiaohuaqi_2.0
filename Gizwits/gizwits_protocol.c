...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagH_1)
    {
        info->event[info->num] = EVENT_H_1;
        info->num++;
        dataPoints->valueH_1 = gizStandardDecompressionValue(H_1_BYTEOFFSET,H_1_BITOFFSET,H_1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagH_2)
    {
        info->event[info->num] = EVENT_H_2;
        info->num++;
        dataPoints->valueH_2 = gizStandardDecompressionValue(H_2_BYTEOFFSET,H_2_BITOFFSET,H_2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagH_3)
    {
        info->event[info->num] = EVENT_H_3;
        info->num++;
        dataPoints->valueH_3 = gizStandardDecompressionValue(H_3_BYTEOFFSET,H_3_BITOFFSET,H_3_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagKG_1)
    {
        info->event[info->num] = EVENT_KG_1;
        info->num++;
        dataPoints->valueKG_1 = gizStandardDecompressionValue(KG_1_BYTEOFFSET,KG_1_BITOFFSET,KG_1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagKG_2)
    {
        info->event[info->num] = EVENT_KG_2;
        info->num++;
        dataPoints->valueKG_2 = gizStandardDecompressionValue(KG_2_BYTEOFFSET,KG_2_BITOFFSET,KG_2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagKG_3)
    {
        info->event[info->num] = EVENT_KG_3;
        info->num++;
        dataPoints->valueKG_3 = gizStandardDecompressionValue(KG_3_BYTEOFFSET,KG_3_BITOFFSET,KG_3_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagKG_4)
    {
        info->event[info->num] = EVENT_KG_4;
        info->num++;
        dataPoints->valueKG_4 = gizStandardDecompressionValue(KG_4_BYTEOFFSET,KG_4_BITOFFSET,KG_4_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagKG_5)
    {
        info->event[info->num] = EVENT_KG_5;
        info->num++;
        dataPoints->valueKG_5 = gizStandardDecompressionValue(KG_5_BYTEOFFSET,KG_5_BITOFFSET,KG_5_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagHT_FZ)
    {
        info->event[info->num] = EVENT_HT_FZ;
        info->num++;
        dataPoints->valueHT_FZ = gizX2Y(HT_FZ_RATIO,  HT_FZ_ADDITION, issuedData->attrVals.valueHT_FZ); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueH_1 != cur->valueH_1)
    {
        GIZWITS_LOG("valueH_1 Changed\n");
        ret = 1;
    }
    if(last->valueH_2 != cur->valueH_2)
    {
        GIZWITS_LOG("valueH_2 Changed\n");
        ret = 1;
    }
    if(last->valueH_3 != cur->valueH_3)
    {
        GIZWITS_LOG("valueH_3 Changed\n");
        ret = 1;
    }
    if(last->valueKG_1 != cur->valueKG_1)
    {
        GIZWITS_LOG("valueKG_1 Changed\n");
        ret = 1;
    }
    if(last->valueKG_2 != cur->valueKG_2)
    {
        GIZWITS_LOG("valueKG_2 Changed\n");
        ret = 1;
    }
    if(last->valueKG_3 != cur->valueKG_3)
    {
        GIZWITS_LOG("valueKG_3 Changed\n");
        ret = 1;
    }
    if(last->valueKG_4 != cur->valueKG_4)
    {
        GIZWITS_LOG("valueKG_4 Changed\n");
        ret = 1;
    }
    if(last->valueKG_5 != cur->valueKG_5)
    {
        GIZWITS_LOG("valueKG_5 Changed\n");
        ret = 1;
    }
    if(last->valueHT_FZ != cur->valueHT_FZ)
    {
        GIZWITS_LOG("valueHT_FZ Changed\n");
        ret = 1;
    }

    if(last->valueH1_ht != cur->valueH1_ht)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueH1_ht Changed\n");
            ret = 1;
        }
    }
    if(last->valueH2_ht != cur->valueH2_ht)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueH2_ht Changed\n");
            ret = 1;
        }
    }
    if(last->valueH3_ht != cur->valueH3_ht)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueH3_ht Changed\n");
            ret = 1;
        }
    }
    if(last->valueMR_FZ != cur->valueMR_FZ)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMR_FZ Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(H_1_BYTEOFFSET,H_1_BITOFFSET,H_1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueH_1);
    gizStandardCompressValue(H_2_BYTEOFFSET,H_2_BITOFFSET,H_2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueH_2);
    gizStandardCompressValue(H_3_BYTEOFFSET,H_3_BITOFFSET,H_3_LEN,(uint8_t *)devStatusPtr,dataPoints->valueH_3);
    gizStandardCompressValue(KG_1_BYTEOFFSET,KG_1_BITOFFSET,KG_1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueKG_1);
    gizStandardCompressValue(KG_2_BYTEOFFSET,KG_2_BITOFFSET,KG_2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueKG_2);
    gizStandardCompressValue(KG_3_BYTEOFFSET,KG_3_BITOFFSET,KG_3_LEN,(uint8_t *)devStatusPtr,dataPoints->valueKG_3);
    gizStandardCompressValue(KG_4_BYTEOFFSET,KG_4_BITOFFSET,KG_4_LEN,(uint8_t *)devStatusPtr,dataPoints->valueKG_4);
    gizStandardCompressValue(KG_5_BYTEOFFSET,KG_5_BITOFFSET,KG_5_LEN,(uint8_t *)devStatusPtr,dataPoints->valueKG_5);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueHT_FZ = gizY2X(HT_FZ_RATIO,  HT_FZ_ADDITION, dataPoints->valueHT_FZ); 
    devStatusPtr->valueH1_ht = gizY2X(H1_ht_RATIO,  H1_ht_ADDITION, dataPoints->valueH1_ht); 
    devStatusPtr->valueH2_ht = gizY2X(H2_ht_RATIO,  H2_ht_ADDITION, dataPoints->valueH2_ht); 
    devStatusPtr->valueH3_ht = gizY2X(H3_ht_RATIO,  H3_ht_ADDITION, dataPoints->valueH3_ht); 
    devStatusPtr->valueMR_FZ = gizY2X(MR_FZ_RATIO,  MR_FZ_ADDITION, dataPoints->valueMR_FZ); 
